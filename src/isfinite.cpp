...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "<cmath> function, isfinite.\n";
    descr += "\n";
    descr += "Checks each element of X if is finite (neither nan nor inf).\n";
    descr += "For complex input, output is true if both real and imag part is finite.\n";
    descr += "Integer data types are not supported.\n";
    descr += "\n";
    descr += "Output (Y) has the same size as X with int8_t data type (since boolean).\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ isfinite X -o Y \n";
    descr += "$ isfinite X > Y \n";
    descr += "$ cat X | isfinite > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        try { transform(begin(X),end(X),begin(Y),[](float x){return isfinite(x);}); }
    ...
    {
        valarray<double> X(i1.N()); valarray<int8_t> Y(o1.N());
        try { ifs1.read(reinterpret_cast<char*>(&X[0]),i1.nbytes()); }
        catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem reading input file (X)" << endl; return 1; }
        try { transform(begin(X),end(X),begin(Y),[](double x){return isfinite(x);}); }
        catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem during call to function" << endl; return 1; }
        if (Y.size()!=o1.N()) { cerr << progstr+": " << __LINE__ << errstr << "unexpected output size" << endl; return 1; }
        if (wo1)
        {
    ...
    {
        valarray<long double> X(i1.N()); valarray<int8_t> Y(o1.N());
        try { ifs1.read(reinterpret_cast<char*>(&X[0]),i1.nbytes()); }
        catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem reading input file (X)" << endl; return 1; }
        try { transform(begin(X),end(X),begin(Y),[](long double x){return isfinite(x);}); }
        catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem during call to function" << endl; return 1; }
        if (Y.size()!=o1.N()) { cerr << progstr+": " << __LINE__ << errstr << "unexpected output size" << endl; return 1; }
        if (wo1)
        {
    ...
    else if (i1.T==101)
    {
        ...
        try { transform(begin(X),end(X),begin(Y),[](complex<float> x){return (isfinite(x.real()) && isfinite(x.imag()));}); }
        catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem during call to function" << endl; return 1; }
        if (Y.size()!=o1.N()) { cerr << progstr+": " << __LINE__ << errstr << "unexpected output size" << endl; return 1; }
        if (wo1)
        {
    ...
    {
        valarray<complex <double>> X(i1.N()); valarray<int8_t> Y(o1.N());
        try { ifs1.read(reinterpret_cast<char*>(&X[0]),i1.nbytes()); }
        catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem reading input file (X)" << endl; return 1; }
        try { transform(begin(X),end(X),begin(Y),[](complex<double> x){return (isfinite(x.real()) && isfinite(x.imag()));}); }
        catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem during call to function" << endl; return 1; }
        if (Y.size()!=o1.N()) { cerr << progstr+": " << __LINE__ << errstr << "unexpected output size" << endl; return 1; }
        if (wo1)
        {
    ...
    {
        valarray<complex <long double>> X(i1.N()); valarray<int8_t> Y(o1.N());
        try { ifs1.read(reinterpret_cast<char*>(&X[0]),i1.nbytes()); }
        catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem reading input file (X)" << endl; return 1; }
        try { transform(begin(X),end(X),begin(Y),[](complex<long double> x){return (isfinite(x.real()) && isfinite(x.imag()));}); }
        catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem during call to function" << endl; return 1; }
        if (Y.size()!=o1.N()) { cerr << progstr+": " << __LINE__ << errstr << "unexpected output size" << endl; return 1; }
        if (wo1)
        {
